...
void ChangeLevel(GameInfo *info) {
  if (info->speed <= 0.1 || info->level >= 10) {
    info->speed = 0.1;
    info->level = 10;
  } else {
    if (info->score - info->previousScore >= 600) {
      double level = info->score;
      double speed = info->score - info->previousScore;
      speed = floor(speed / 600);
      level = floor(level / 600);
      info->level = level + 1;
      info->speed -= 0.1 * speed;
...
int AllocateMemory(GameInfo *info) {
  int result = 1;
  info->array = calloc(HEIGHT, sizeof(int *));
  if (info->array == NULL) {
    return 0;
  }
  for (int i = 0; i < HEIGHT; i++) {
    info->array[i] = calloc(WIDTH, sizeof(int));
